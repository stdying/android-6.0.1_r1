...
import com.example.android.rs.vr.engine.bugdroidBitCode;

/**
 * @hide
 */
public class ScriptC_bugdroid extends ScriptC {
    private static final String __rs_resource_name = "bugdroid";
    // Constructor
    public  ScriptC_bugdroid(RenderScript rs) {
        super(rs,
              __rs_resource_name,
              bugdroidBitCode.getBitCode32(),
              bugdroidBitCode.getBitCode64());
    ...
    private final static int mExportForEachIdx_andy = 1;
    public Script.KernelID getKernelID_andy() {
        return createKernelID(mExportForEachIdx_andy, 58, null, null);
    }

    public void forEach_andy(Allocation aout) {
        forEach_andy(aout, null);
    }

    public void forEach_andy(Allocation aout, Script.LaunchOptions sc) {
        // check aout
        if (!aout.getType().getElement().isCompatible(__I16)) {
            throw new RSRuntimeException("Type mismatch with I16!");
        }
        forEach(mExportForEachIdx_andy, (Allocation) null, aout, null, sc);
    }
