...
public class ScriptField_DifferentDefinition8 extends android.renderscript.Script.FieldBase {
    ...
    private  ScriptField_DifferentDefinition8(RenderScript rs) {
        mItemArray = null;
        mIOBuffer = null;
    ...
    public  ScriptField_DifferentDefinition8(RenderScript rs, int count, int usages) {
        mItemArray = null;
        mIOBuffer = null;
        mElement = createElement(rs);
    ...
    public static ScriptField_DifferentDefinition8 create1D(RenderScript rs, int dimX, int usages) {
        ScriptField_DifferentDefinition8 obj = new ScriptField_DifferentDefinition8(rs);
        obj.mAllocation = Allocation.createSized(rs, obj.mElement, dimX, usages);
        return obj;
    }

    public static ScriptField_DifferentDefinition8 create1D(RenderScript rs, int dimX) {
        return create1D(rs, dimX, Allocation.USAGE_SCRIPT);
    }

    public static ScriptField_DifferentDefinition8 create2D(RenderScript rs, int dimX, int dimY) {
        return create2D(rs, dimX, dimY, Allocation.USAGE_SCRIPT);
    }

    public static ScriptField_DifferentDefinition8 create2D(RenderScript rs, int dimX, int dimY, int usages) {
        ScriptField_DifferentDefinition8 obj = new ScriptField_DifferentDefinition8(rs);
    ...
    public static ScriptField_DifferentDefinition8 createCustom(RenderScript rs, Type.Builder tb, int usages) {
        ScriptField_DifferentDefinition8 obj = new ScriptField_DifferentDefinition8(rs);
        Type t = tb.create();
    ...
    public synchronized void set_member1(int index, ScriptField_DifferentDefinition7.Item v, boolean copyNow) {
        if (mIOBuffer == null) mIOBuffer = new FieldPacker(mElement.getBytesSize() * getType().getX()/* count */);
        if (mItemArray == null) mItemArray = new Item[getType().getX() /* count */];
    ...
    public synchronized ScriptField_DifferentDefinition7.Item get_member1(int index) {
        if (mItemArray == null) return null;
