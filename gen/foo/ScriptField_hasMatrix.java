 ...
 * @hide
 */
public class ScriptField_hasMatrix extends android.renderscript.Script.FieldBase {
    ...
    public static Element createElement(RenderScript rs) {
        ...
        eb.add(Element.MATRIX_3X3(rs), "m");
        eb.add(Element.I32(rs), "i");
        return eb.create();
    }

    private  ScriptField_hasMatrix(RenderScript rs) {
        mItemArray = null;
        mIOBuffer = null;
        mElement = createElement(rs);
    }

    public  ScriptField_hasMatrix(RenderScript rs, int count) {
        mItemArray = null;
        mIOBuffer = null;
        mElement = createElement(rs);
        init(rs, count);
    ...
    public  ScriptField_hasMatrix(RenderScript rs, int count, int usages) {
        mItemArray = null;
        mIOBuffer = null;
        mElement = createElement(rs);
    ...
    public static ScriptField_hasMatrix create1D(RenderScript rs, int dimX, int usages) {
        ScriptField_hasMatrix obj = new ScriptField_hasMatrix(rs);
        obj.mAllocation = Allocation.createSized(rs, obj.mElement, dimX, usages);
        return obj;
    }

    public static ScriptField_hasMatrix create1D(RenderScript rs, int dimX) {
        return create1D(rs, dimX, Allocation.USAGE_SCRIPT);
    }

    public static ScriptField_hasMatrix create2D(RenderScript rs, int dimX, int dimY) {
        return create2D(rs, dimX, dimY, Allocation.USAGE_SCRIPT);
    }

    public static ScriptField_hasMatrix create2D(RenderScript rs, int dimX, int dimY, int usages) {
        ScriptField_hasMatrix obj = new ScriptField_hasMatrix(rs);
    ...
    public static ScriptField_hasMatrix createCustom(RenderScript rs, Type.Builder tb, int usages) {
        ScriptField_hasMatrix obj = new ScriptField_hasMatrix(rs);
        Type t = tb.create();
    ...
    public synchronized void set_m(int index, Matrix3f v, boolean copyNow) {
        if (mIOBuffer == null) mIOBuffer = new FieldPacker(mElement.getBytesSize() * getType().getX()/* count */);
        if (mItemArray == null) mItemArray = new Item[getType().getX() /* count */];
        if (mItemArray[index] == null) mItemArray[index] = new Item();
    ...
    public synchronized Matrix3f get_m(int index) {
        if (mItemArray == null) return null;
