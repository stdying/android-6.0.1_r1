 ...
 * @hide
 */
public class ScriptField_new_struct extends android.renderscript.Script.FieldBase {
    static public class Item {

        ScriptField_new_matrix.Item m;
        Matrix3f t;
        int i;

        Item() {
            m = new ScriptField_new_matrix.Item();
            t = new Matrix3f();
        }
    ...
    public static Element createElement(RenderScript rs) {
        ...
        eb_.add(Element.F32(rs), "f", 2);
        eb.add(eb_.create(), "m");
        eb.add(Element.MATRIX_3X3(rs), "t");
        eb.add(Element.I32(rs), "i");
        return eb.create();
    }

    private  ScriptField_new_struct(RenderScript rs) {
        mItemArray = null;
        mIOBuffer = null;
        mElement = createElement(rs);
    }

    public  ScriptField_new_struct(RenderScript rs, int count) {
        mItemArray = null;
        mIOBuffer = null;
        mElement = createElement(rs);
        init(rs, count);
    ...
    public  ScriptField_new_struct(RenderScript rs, int count, int usages) {
        mItemArray = null;
        mIOBuffer = null;
        mElement = createElement(rs);
    ...
    public static ScriptField_new_struct create1D(RenderScript rs, int dimX, int usages) {
        ScriptField_new_struct obj = new ScriptField_new_struct(rs);
        obj.mAllocation = Allocation.createSized(rs, obj.mElement, dimX, usages);
        return obj;
    }

    public static ScriptField_new_struct create1D(RenderScript rs, int dimX) {
        return create1D(rs, dimX, Allocation.USAGE_SCRIPT);
    }

    public static ScriptField_new_struct create2D(RenderScript rs, int dimX, int dimY) {
        return create2D(rs, dimX, dimY, Allocation.USAGE_SCRIPT);
    }

    public static ScriptField_new_struct create2D(RenderScript rs, int dimX, int dimY, int usages) {
        ScriptField_new_struct obj = new ScriptField_new_struct(rs);
    ...
    public static ScriptField_new_struct createCustom(RenderScript rs, Type.Builder tb, int usages) {
        ScriptField_new_struct obj = new ScriptField_new_struct(rs);
        Type t = tb.create();
    ...
    public synchronized void set_m(int index, ScriptField_new_matrix.Item v, boolean copyNow) {
        if (mIOBuffer == null) mIOBuffer = new FieldPacker(mElement.getBytesSize() * getType().getX()/* count */);
        if (mItemArray == null) mItemArray = new Item[getType().getX() /* count */];
        if (mItemArray[index] == null) mItemArray[index] = new Item();
        mItemArray[index].m = v;
        if (copyNow)  {
            mIOBuffer.reset(index * mElement.getBytesSize());
            for (int ct2 = 0; ct2 < 2; ct2++) {
                mIOBuffer.addF32(v.f[ct2]);
            }
    ...
    public synchronized void set_t(int index, Matrix3f v, boolean copyNow) {
        if (mIOBuffer == null) mIOBuffer = new FieldPacker(mElement.getBytesSize() * getType().getX()/* count */);
        if (mItemArray == null) mItemArray = new Item[getType().getX() /* count */];
        if (mItemArray[index] == null) mItemArray[index] = new Item();
        mItemArray[index].t = v;
        if (copyNow)  {
    ...
    public synchronized ScriptField_new_matrix.Item get_m(int index) {
        if (mItemArray == null) return null;
        return mItemArray[index].m;
    }

    public synchronized Matrix3f get_t(int index) {
        if (mItemArray == null) return null;
        return mItemArray[index].t;
    }
